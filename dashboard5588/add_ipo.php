...
if (isset($_COOKIE['edit_id']) || isset($_COOKIE['view_id'])) {
    ...
    $stmt = $obj->con1->prepare("SELECT * FROM `ipo` WHERE id=?");
...
if (isset($_REQUEST["save"])) {
    ...
    if ($doc != "") {
        if (file_exists("documents/ipo/" . $doc)) {
            $i = 0;
            $DocFileName = $doc;
            $Arr1 = explode('.', $DocFileName);

            $DocFileName = $Arr1[0] . $i . "." . $Arr1[1];
            while (file_exists("documents/ipo/" . $DocFileName)) {
                $i++;
                $DocFileName = $Arr1[0] . $i . "." . $Arr1[1];
            }
        } else {
    ...
    try {
        $stmt = $obj->con1->prepare("INSERT INTO `ipo`(`title`, `file`, `status`) VALUES (?,?,?)");
    ...
    if ($Resp) {
        move_uploaded_file($doc_path, "documents/ipo/" . $DocFileName);
...
if (isset($_REQUEST["update"])) {
    ...
    try {
        $stmt = $obj->con1->prepare("UPDATE `ipo` SET `title`=?,`file`=?, `status`=? WHERE `id`=?");
                ...
                <div class="card-body">
                    ...
                    <form class="row g-3 pt-3" method="post" enctype="multipart/form-data">
                        ...
                        <div class="text-left mt-4">
                            ...
                            <button type="button" class="btn btn-danger" onclick="window.location='ipo.php'">
                                Close</button>
                        </div>
                    </form>
                </div>
