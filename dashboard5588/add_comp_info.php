...
if (isset($_COOKIE['edit_id']) || isset($_COOKIE['view_id'])) {
    ...
    $stmt = $obj->con1->prepare("SELECT * FROM `comp_info` WHERE  id=?");
...
if (isset($_REQUEST["save"])) {
    ...
    try {
        $stmt = $obj->con1->prepare("INSERT INTO `comp_info`(`name`,`description`) VALUES (?,?)");
    ...
    if ($Resp) {
        ...
        if (isset($_REQUEST["update"])) {
            ...
            $description=$_REQUEST["description"];
            try {
                $stmt = $obj->con1->prepare("UPDATE `comp_info` SET `name`=?,`description`=? WHERE `id`=?");
    ...
    <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">
                <?php echo (isset($mode)) ? (($mode == 'view') ? 'View' : 'Edit') : 'Add' ?>-Company Info</li>
        </ol>
    </nav>
</div><!-- End Page Title -->
