...
if (isset($_REQUEST["btndelete"])) {
    $id = $_REQUEST['delete_id'];
     ...
     try {
         $stmt_doc = $obj->con1->prepare("SELECT * FROM `ipo` WHERE id=?");
         $stmt_doc->bind_param("i",$id);
         $stmt_doc->execute();
         $Resp_doc = $stmt_doc->get_result()->fetch_assoc();
         $stmt_doc->close();
 
         if (file_exists("documents/ipo/" . $Resp_doc["file"])) {
             unlink("documents/ipo/" . $Resp_doc["file"]);
         }
 
         $stmt_del = $obj->con1->prepare("DELETE FROM `ipo` WHERE id=?");
...
<div class="modal fade" id="deleteModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="post" action="ipo.php">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table class="table datatable">
                        ...
                        <tbody>
                            <?php
                $stmt_list = $obj->con1->prepare("SELECT * FROM `ipo`ORDER BY `id` DESC;");
                $stmt_list->execute();
